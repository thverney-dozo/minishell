...
/*   Updated: 2020/02/03 21:38:53 by thverney         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (!ft_strncmp(cmd, "exit", 5) || !ft_strncmp(cmd, "exit ", 5) || !ft_strncmp(cmd, "exit|", 5))
		indic == 1 ? exit (1) : 0;
    else if (!(ft_strncmp(cmd, "echo ", 5)) || !ft_strncmp(cmd, "echo", 5))
...
void	ft_pipe_is_cmd(t_env *env, int i)
{
	...
	|| env->pos_prev[i] <= 32 || env->pos_prev[i] == '|'))
		i++;
	if (env->pos_prev[i] == '\0')
	{
...
void	is_pipe_here(t_env *env)
{
	...
	env->pipe_here = (env->pos_prev[i] == '|' ? 1 : 0);
}