...
/*   Updated: 2020/02/25 08:22:37 by thverney         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		split_pipe_line_three(int i, t_env *env, t_cmd *cmd)
{
	if (env->line[i] == 92 && env->line[i + 1] == 92)
		fill_triple_string(env, env->j++, '2', env->line[i++]);
	else if (env->line[i] == '|' && !nbslash(env->line, i, cmd))
		return (i);
	else
	{
		if (env->count > env->j)
...
int		split_pipe_line_two(int i, t_env *env, t_cmd *cmd)
{
	...
	|| env->line[i + 1] == '\\'))
		fill_triple_string(env, env->j, '2', env->line[i]);
	else if (env->line[i] == '$' && !nbslash(env->line, i, cmd) && i++)
	{
	...
	else if (!nbslash(env->line, i, cmd) && (env->line[i] == '>'
	|| env->line[i] == '<'))
	{
		...
		i += stock_file(env->line + i, env->t, env, cmd);
	}
	else
		i = split_pipe_line_three(i, env, cmd);
	return (i);
}
